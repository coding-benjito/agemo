...
@numba.njit
def series_product_alt(arr1, arr2, subsetdict):
	#arr1*arr2
	size = arr1.size
	result = np.zeros_like(arr1)
	for k in range(size):
...
def product_subsetdict_marg_alt(shape, all_mutypes):
	result = numba.typed.Dict()
...
def product_f(subsetdict, f):
	if len(f)==1:
		...
		result = f[0] 
		for f2 in f[1:]: 
			result = series_product_alt(result, f2, subsetdict)
		return result
...
def product_f_g(subsetdict, f, g, signs):
	if g.shape[0]==0:
		...
		for idx, (fs, gs, sign) in enumerate(zip(f, g, signs)):
			result[idx] = sign * series_product_alt(fs, gs, subsetdict)
		return result
	...
	if num_equations==0:
		...
		for idx, eq in enumerate(eq_matrix):
			...
			for idx2, mutype in enumerate(mutype_array):
				...
				result[idx, idx2] = taylor_coeff_inverse_polynomial_alt(eq, theta, mutype, num_branchtypes, dot_product, mutype_shape)
		return result
...
def all_exponentials_alt(eq_matrix, size, var_array, time, num_branchtypes, mutypes, mutype_shape):
	#eq_matrix contains only denominators!
	num_equations = eq_matrix.shape[0]
	...
	for idx, eq in enumerate(eq_matrix):
		...
		for idx2, mutype in enumerate(mutypes):
			result[idx, idx2] = taylor_coeff_exponential_alt(-time, eq, dot_product, mutype, num_branchtypes, theta, mutype_shape)
	return result
...
def compile_inverted_eq_alt(eq_matrix, size, subsetdict, delta_in_nom, mutype_array, mutype_shape):
	#delta_column should already have been removed from eq_matrix
	#note we need to add np.zeros(eq_matrix.shape[-1], dtype=int) 
	#to eq_matrix if no delta in numerator
	#polyf poles should be pairwise differences n*(n-1)/2 for n poles
	...
	def _make_eq(var, time):    
		...
		#make derivative matrix for all pairwise differences eq_matrix 
		try:
			polyf = all_polynomials_alt(eq_matrix_diffs, size, var, num_branchtypes, mutype_array, mutype_shape)
			#combine derivative matrix results
			denoms = product_pairwise_diff_inverse_polynomial(
				polyf, (num_terms, size), pairwise_idxs, subsetdict
				)
			#n terms, both expf and denoms should be of length n
			expf = all_exponentials_alt(denominators_eq, size, var, time, num_branchtypes, mutype_array, mutype_shape)
			#adapt with signs! diff dims!
...
def compile_non_inverted_eq_alt(eq_matrix, size, mutype_array, mutype_shape):
	num_branchtypes = len(mutype_shape)
	def _make_eq(var):
		result = np.zeros(size, np.float64)
		constants = eq_matrix[:,0].dot(var)
		result = all_polynomials_alt(eq_matrix[:,1], size, var, num_branchtypes, mutype_array, mutype_shape)
		transpose = result.T 
		transpose *= constants
		return result
	return _make_eq
...
def prepare_graph_evaluation_with_marginals_alt(eq_matrix, to_invert_array, eq_array, size, delta_idx, subsetdict, mutype_array, mutype_shape):
	#generates function for each node of the equation graph
	if delta_idx is None:
		eq_matrix_no_delta = eq_matrix
	else:		
	...
	f_non_inverted = compile_non_inverted_eq_alt(eq_matrix_no_delta[not_to_invert], size, mutype_array, mutype_shape)
	f_inverted = []
	for idx in range(i, len(to_invert_array)):
		eq_idxs = eq_array[idx]
		eq_idxs = np.array(eq_idxs, dtype=int)
		delta_in_nom = np.any(eq_matrix[eq_idxs][:, 0, delta_idx])==1
		f_inverted.append(compile_inverted_eq_alt(eq_matrix_no_delta[eq_idxs], size, subsetdict, delta_in_nom, mutype_array, mutype_shape))
	return (f_non_inverted, f_inverted)

def evaluate_single_point_with_marginals_alt(size, num_eq_tuple, f_tuple):
...
def taylor_to_probability_coeffs_alt(mutype_array, mutype_shape, include_marginals=False):
	if not include_marginals:
...
@numba.njit
def iterate_eq_graph_alt(sequence, graph, evaluated_eqs, subsetdict):
	...
	for parent in sequence:
		...
		if len(children)>0:
			for child in children:
				temp+=node_values[child]
			if parent!=0:
				node_values[parent] = series_product_alt(temp, evaluated_eqs[parent-1], subsetdict)
			else:
				node_values[parent] = temp
	return node_values[0]
